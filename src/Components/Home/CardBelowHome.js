...
const CardBelowHome = () => {
    const adoptedPets = formatNumber(10000);
  return (
    <div className='dark-grey-container'>
      <div className='left-pic'><img src={HomeDarkCardLeftPic} alt="Dog with toy"/></div>
      <div className='left-para'><p><p className='adopted-pets-num'>{adoptedPets}</p> Furry Buddies<br/>Living Their Best Lives</p></div>
      <div className='right-pic'><img src={HomeDarkCardRightPic} alt="Dog pic" /></div>
      <div className='right-para'><p className='we-do'>WHAT WE DO?</p>With a focus on matching the right pet with the right family, AdoptUs makes it easy to adopt love and foster happiness.</div>
    </div>
  )
}
